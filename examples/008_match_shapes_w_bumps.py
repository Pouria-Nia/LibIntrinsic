...
def main():
    print("--- Shape Matching Experiment (With Noise) ---")
    
    # 1. Create Source Shape
    print("1. Generating Shape A...")
    shape_a_viz = trimesh.creation.torus(major_radius=1.0, minor_radius=0.4, major_sections=32, minor_sections=32)
    
    # --- THE FIX: Add Noise to break symmetry ---
    # This gives every vertex a unique "bumpiness"
    print("   -> Adding geometric noise to make vertices unique...")
    noise_amplitude = 0.02
    noise = np.random.normal(0, noise_amplitude, shape_a_viz.vertices.shape)
    shape_a_viz.vertices += noise
    # ---------------------------------------------

    # Process Shape A
    mesh_a, hks_a = process_shape(shape_a_viz)
    
    # 2. Create Target Shape (Copy the noisy shape, THEN rotate/move)
    print("2. Generating Shape B (Rotated & Moved)...")
    shape_b_viz = shape_a_viz.copy()
    
    # Apply transformation
    matrix = trimesh.transformations.random_rotation_matrix()
    matrix[0:3, 3] = [2.0, 2.0, 2.0]
